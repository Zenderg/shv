...
- If, while doing any task, you encounter a very large file that should be decomposed, decompose it immediately even when that work is unrelated to the current task; do not wait for separate approval. Treat a file as decomposition-worthy when its size and mixed responsibilities materially harm navigation, review, or maintainability, rather than by line count alone. Preserve existing behavior and public contracts, perform proportionate verification, and commit the decomposition separately with a `refactor:` conventional commit. Stage only the decomposition changes and never include unrelated user changes. An explicit user instruction not to modify or commit takes precedence.
