...
Every URL supplied to the downloader, including subtitle, HLS, and DASH URLs derived from manifests, must use HTTP(S) and must not contain embedded credentials. All Node, browser-impersonated, and ffmpeg media traffic passes through a loopback media proxy. The proxy resolves each hostname with system DNS and connects directly to an address from that DNS snapshot, preserving the original hostname for HTTP Host and TLS and avoiding a second lookup between resolution and connection. It intentionally does not classify reachable destinations as public or private: in the supported trusted LAN/VPN deployment, container DNS and routing define media reachability. Node-managed downloads follow redirects manually and recompute captured headers for each target origin. Browser-impersonated inputs disable redirects. Each ffmpeg DASH input receives its own origin-locked proxy, so same-origin redirects remain usable but a redirect to another origin is rejected before ffmpeg can replay captured credentials. The HLS ffmpeg fallback uses the general media proxy but receives no captured request headers because ffmpeg refetches a mutable manifest whose child origins can differ from the inspected snapshot.
