...
export function createRouter(services: RouteServices): Router {
  ...
  router.get('/api/jobs/:id/candidates', (request, response) => {
    response.json(candidateResponses(services.jobs.listCandidates(request.params.id)));
  });
  ...
  router.post('/api/jobs/:id/extension-candidates', (request, response) => {
    ...
    response.json(candidateResponses(services.jobs.replaceExtensionCandidates(request.params.id, body.candidates)));
  });
